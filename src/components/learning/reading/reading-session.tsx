...
import { submitReadingAnswersAction } from "@/server/learning/reading.actions";
import type { ReadingDetail, ReadingSubmissionResult } from "@/types/learning";

type ReadingSessionProps = {
  reading: ReadingDetail;
};

export function ReadingSession({ reading }: ReadingSessionProps) {
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const response = await submitReadingAnswersAction({
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-6">
        <section className="rounded-xl border border-border bg-card p-6 shadow-sm">
          ...
          <p className="mt-2 text-muted-foreground">{reading.title}</p>
          ...
          <p className="mt-4 text-sm text-muted-foreground">
            Reading mastery updated to {result.masteryPercent}%
          </p>
          <div className="mt-6 flex flex-wrap gap-2">
            ...
            <Link
              href="/app/learn/reading"
              className="inline-flex items-center justify-center rounded-lg border border-border bg-secondary px-5 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border focus-visible:ring-offset-2"
            >
              Back to reading
            </Link>
          </div>
  ...
  return (
    <div className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
      ...
      >
        <h2 id="passage-heading" className="sr-only">
          Reading passage
        </h2>
        <div className="max-w-prose">
          {passageLines.map((line, index) => (
            <p
              key={`${index}-${line}`}
              className="font-japanese text-lg leading-8 text-foreground"
            >
              {line}
            </p>
          ))}
